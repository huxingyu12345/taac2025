...
from hstu_modules import HSTUEncoder
from time_features import TimeFeatureEmbedding
...
class BaselineModel(torch.nn.Module):
    ...
    Attributes:
        ...
        hstu_encoder: HSTU编码器，替换原有的Transformer层
    """
    ...
    def __init__(self, user_num, item_num, feat_statistics, feat_types, args):  #
        ...
        self.temp = args.temperature  # 从 args 获取 temperature
        self.dev = args.device
        ...
        # 使用HSTU编码器替换原有的Transformer层
        self.hstu_encoder = HSTUEncoder(
            embedding_dim=args.hidden_units,
            num_blocks=args.num_blocks,
            num_heads=args.num_heads,
            linear_hidden_dim=args.hidden_units,  # HSTU的线性隐藏维度
            attention_dim=args.hidden_units // args.num_heads,  # 注意力维度
            dropout_ratio=args.dropout_rate,
            linear_activation="silu",  # HSTU特有的SiLU激活函数
            enable_relative_bias=True,  # 启用相对位置偏置
    ...
    def feat2emb(self, seq, feature_array, mask=None, include_user=False):
        ...
        if include_user:
            ...
            item_feat_list = [item_embedding]
            user_feat_list = []  # 初始化为空列表，防止未定义错误
    ...
    def log2feats(self, log_seqs, mask, seq_feature):
        ...
        attention_mask_tril = torch.tril(ones_matrix)  # 下三角矩阵
        attention_mask_pad = (mask != 0).to(self.dev)  # padding掩码
        # 组合因果掩码和padding掩码
        attention_mask = attention_mask_tril.unsqueeze(0) & attention_mask_pad.unsqueeze(1)
        attention_mask = attention_mask.float()  # HSTU需要float类型

        # 4. 使用HSTU编码器替换原有的Transformer层
        log_feats = self.hstu_encoder(seqs, attention_mask)

        return log_feats

    def compute_infonce_loss(self, seq_embs, pos_embs, neg_embs, loss_mask, writer, global_step):
        """
        计算 InfoNCE 损失，使用归一化余弦相似度和交叉熵
        Args:
            seq_embs: 序列嵌入, [batch_size, maxlen, hidden_units]
            pos_embs: 正样本嵌入, [batch_size, maxlen, hidden_units]
            neg_embs: 负样本嵌入, [batch_size, maxlen, hidden_units]
            loss_mask: 损失掩码, [batch_size, maxlen]
            writer: SummaryWriter对象，用于记录日志
            global_step: 全局步数，用于 TensorBoard
        Returns:
            loss: InfoNCE 损失，标量
        """
        hidden_size = seq_embs.size(-1)
        batch_size, maxlen = seq_embs.size()[:2]
        ...
        # 计算正样本余弦相似度 [batch_size, maxlen, 1]
        ...
        # 计算负样本相似度 [batch_size, maxlen, batch_size * maxlen]
        neg_embedding_all = neg_embs.reshape(-1, hidden_size)  # [batch_size * maxlen, hidden_size]
        neg_logits = torch.matmul(seq_embs, neg_embedding_all.transpose(-1, -2))  # [batch_size, maxlen, batch_size * maxlen]
        writer.add_scalar("Model/nce_neg_logits", neg_logits.mean().item(), global_step)

        # 合并正负样本 logits
        logits = torch.cat([pos_logits, neg_logits], dim=-1)  # [batch_size, maxlen, batch_size * maxlen + 1]
        
        # 调试：打印形状
        print(f"logits shape: {logits.shape}, expected: [{batch_size}, {maxlen}, {batch_size * maxlen + 1}]")

        # 仅对有效位置 (loss_mask == 1) 计算损失
        loss_mask = loss_mask.bool()
        logits = logits[loss_mask] / self.temp  # [valid_positions, batch_size * maxlen + 1]
        if logits.size(0) == 0:
            return torch.tensor(0.0, device=self.dev, requires_grad=True)

        labels = torch.zeros(logits.size(0), device=self.dev, dtype=torch.int64)
